...
- **LeanSpec**：可通过 MCP 将 Spec 暴露给助手（配置示例见 [指南 - AI 集成](https://www.lean-spec.dev/zh-Hans/docs/guide/)）。  
- **planning-with-files**：依赖 Cursor hooks + 规则自动提醒读计划、写 progress；与 LeanSpec MCP **并行不冲突**，注意上下文里优先读 **当前 effective** 三文件 + **当前 SpecRef**。
